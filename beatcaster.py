...
def do_work(track, options):
    ...
    for beat in beats[:-1]:
        rate = float(options.slowdown)/int(options.pattern[n_beat % len(options.pattern)])
    	last_start = start
        start = int(beat.start * track.sampleRate)
        if options.debug and (n_beat % len(options.pattern))==(len(options.pattern)-1) and (n_beat > 0):
