...
var WebAudio = {
  ...
  loadSound: function(aaa, cb) {
    url = "http://archive.garron.us/temp/shiyam/Audiomachine%20-%20Reaching%20[Dance%20Version].mp3";
...
var Clip = {
  createUI: function(src, offsetX, width, left) {
    var width = width || '100%';
    var clip = $('<div></div>').addClass('clip').css('background-image', 'url("' + src + '")')
...
Wav.createWaveFileData = (function() {
  ...
  var writeAudioBuffer = function(audioBuffer, a, offset) {
      var n = audioBuffer.length,
        ...
        sampleL, bufferR = audioBuffer.getChannelData(1),
        sampleR;
  ...
  handle["blend"] = function(audioBuffer, a, offset, beat, overlap) {
    ...
    beat["segment1"]["end"] - beat["segment1"]["start"], beat["segment2"]["end"] - beat["segment2"]["start"]));

    offset = handle["copy"](audioBuffer, a, offset, {
      "kind": "copy",
      "segment": {
...
$(window).bind('hashchange load', function() {
  ...
  if(!window['webkitAudioContext']) {
    $('.track.source').html('<h1>You need to be running the latest version of Chrome. Get it <a href="http://www.google.com/chrome">here</a></h1>').addClass('loaded');
  ...
  $.getJSON(soundUrl + '.json', function(sound) {
    ...
    $('.track.source').removeClass('loaded').width(0).find('.clip').replaceWith(Clip.createUI(sound.waveform_url));
  ...
  var keyMap = {
    ...
    13: function() { // Enter
      var $sourceTrack = $('.track.source'),
        ...
        sound = Sounds.sources[Sounds.current],
        selection = $sourceTrack.imgAreaSelect({
          instance: true
        }).getSelection(),
        clip = Clip.create(selection, $sourceTrack.width(), sound.duration);

      $resultTrack.append(Clip.createUI(sound.waveform_url, selection.x1, selection.width, $resultTrack.width())).width($resultTrack.width() + selection.width);
      clip.id = sound.id;
  ...
  if(!ev.metaKey && !ev.shiftKey && !ev.ctrlKey && !ev.altKey && ev.target && !/input|textarea/i.test(ev.target.nodeName)) {
  ...
  $('.track.source').imgAreaSelect({
    ...
    onSelectEnd: function(img, selection) {
      ...
      sound.anim = new Animation(WebAudio.context.currentTime, function() {
        ...
        var selectionWidth = $('.track.source').imgAreaSelect({
          instance: true
        }).getSelection().width;
