...
handle["copy"] = function(audioBuffer, a, offset, beat, overlap) {
  ...
  // console.log(beat);
  var startSample = Math.floor(audioBuffer.sampleRate * beat["segment"]["start"]);
...
handle["blend"] = function(audioBuffer, a, offset, beat, overlap) {
  ...
  for(var i = 0; i < num_overlap_samples; ++i) {
    ...
    var sample1L = bufferL[Math.floor(audioBuffer.sampleRate * beat["segment1"]["end"]) - num_overlap_samples + i] * 32768.0;
    var sample1R = bufferR[Math.floor(audioBuffer.sampleRate * beat["segment1"]["end"]) - num_overlap_samples + i] * 32768.0;
    var sample2L = bufferL[Math.floor(audioBuffer.sampleRate * beat["segment2"]["end"]) - num_overlap_samples + i] * 32768.0;
    var sample2R = bufferR[Math.floor(audioBuffer.sampleRate * beat["segment2"]["end"]) - num_overlap_samples + i] * 32768.0;
...
export function createWaveFileData(audioBuffer, hackData, progressCallback) {
