...
Used to re-generate a higher-quality version of the Bohemian National Waltz, Feb. 2011.
"""
...
def do_work(track, options):
    ...
    for beat in beats[:-1]:
        ...
        #print rate1, rate2
        rates.append((start1-offset, rate1*1.25))
        rates.append((start2-offset, rate2*1.25))
...
def main():
    ...
    name = name[0]+'_swing'+sign+str(int(abs(float(options.swing))*100))+'.wav'
    name = name.replace(' ','')
